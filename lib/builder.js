...
module.exports = class Builder {
  ...
  get pages() {
    return this.parse(this.data.pages);
  }
   ...
   */
  get publishedPages() {
    return this.onlyPublished(this.pages);
  }
  ...
  get pagesJson() {
    return this.list(this.pagesDirectory);
  }
  ...
  get pagesDirectory() {
    return mapObject(this.publishedPages, (slug, page) => {
      let authors = values(this.publishedAuthors).map(author => this.doc('author', author));
      let tags = values(this.publishedTags).map(tag => this.doc('tag', tag));

      return {
        data: this.doc('page', page),
        included: [].concat(tags, authors)
      };
    });
  }
  ...
  build() {
    return {
      ...
      'pages': serializeDirectory(this.pagesDirectory),
      'pages.json': serializeFile(this.pagesJson),
