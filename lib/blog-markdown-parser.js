...
BlogMarkdownParser.prototype.processString = function(content, relativePath) {
  let collection = isAuthor(relativePath) ? this.parsedAuthors : this.parsedPosts;
  ...
  let existingPostIndex = collection.map((post) => post.slug).indexOf(parsed.slug);
  if (existingPostIndex >= 0) {
    collection.splice(existingPostIndex, 1, parsed);
  } else {
...
function isAuthor(filePath) {
  return path.dirname(filePath).split(path.sep).pop() === 'authors';
}
